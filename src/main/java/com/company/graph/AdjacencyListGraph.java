...
public class AdjacencyListGraph<V, E> implements Graph<V, E> {
    private class VertexImpl<V> implements Vertex<V> {
        private V element;
        private List<Edge<E>> adjacencyOutgoing, adjacencyIncoming;

        public VertexImpl(V element, boolean isDirected) {
            this.element = element;
            adjacencyOutgoing = new LinkedList<>();
            if (isDirected) {
                adjacencyIncoming = new LinkedList<>();
            } else {
                adjacencyIncoming = adjacencyOutgoing;
            }
        }
        ...
        public boolean validate(Graph<V, E> graph) {
            return (AdjacencyListGraph.this == graph);
        }

        public List<Edge<E>> getOutgoing() {
            return adjacencyOutgoing;
        }

        public List<Edge<E>> getIncoming() {
            return adjacencyIncoming;
        }
    }
    ...
    public Edge<E> insertEdge(Vertex<V> u, Vertex<V> v, E element) throws IllegalArgumentException {
        if (getEdge(u, v) == null) {
            ...
            startVertex.getOutgoing().add(edge);
            endVertex.getIncoming().add(edge);
            return edge;
        } else {
    ...
    public void removeEdge(Edge<E> e) throws IllegalArgumentException {
        ...
        edges.remove(edge);
        Vertex<V>[] vertices = endVertices(edge);
        ((VertexImpl<V>) vertices[0]).getOutgoing().remove(edge);
        ((VertexImpl<V>) vertices[1]).getIncoming().remove(edge);
    }

    public static void main(String[] args) {
        Graph<String, Integer> undirectGraph = new AdjacencyListGraph<>(false);
        ...
        Graph<String, Integer> directGraph = new AdjacencyListGraph<>(true);
    }
}
