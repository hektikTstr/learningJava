...
public class ChainHashMapWithNewHashCode<K, V> extends AbstractHashMap<K, V> {
    // a fixed capacity array of UnsortedTableMap that serve as buckets
    private UnsortedTableMap<K, V>[] table; // initialized within createTable
    public ChainHashMapWithNewHashCode() { super(); }
    public ChainHashMapWithNewHashCode(int cap) { super(cap); }
    public ChainHashMapWithNewHashCode(int cap, int p) { super(cap, p); }

    @Override
    protected int hashValue(K key) {
        return (Math.abs(key.hashCode()) * 6) % 12;
    }
